...
class InboundOrder(Base):
    ...
    id: Mapped[int] = mapped_column("inbound_id", Integer, primary_key=True, autoincrement=True)
...
class Shipment(Base):
    ...
    def actual_cost(self) -> Optional[float]:
        if hasattr(self, "_total_product_value") and self._total_product_value is not None:
            return float(self._total_product_value)
        # Do NOT fall back to shipping_cost — that's a separate field shown as "Shipping"
        return None
