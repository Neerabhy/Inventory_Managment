...
from .api import analytics, auth, copilot, inventory, logistics, procurement, reports, returns
from .core.config import settings
from .core.database import AsyncSessionLocal, init_db
from .models.analytics import KpiDefinition
from .models.auth import Role
from .models.logistics import ServiceableCity
from .services.copilot_session_history import reset_copilot_history


async def _ensure_prediction_tables() -> None:
    """Apply v4 prediction schema if tables are missing."""
    import sqlite3
    from pathlib import Path

    migration = Path(__file__).resolve().parents[1] / "migration_v4_predictions.sql"
    ...
    try:
        ...
        logger.warning(f"Prediction migration skipped: {exc}")
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    await init_db()
    await _ensure_prediction_tables()
    await _seed_roles()
    await _seed_cities()
    await _seed_kpis()
    reset_copilot_history()
