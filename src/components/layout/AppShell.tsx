          ...
          <AnimatePresence>
            {!collapsed && (
              ...
              >
                ...
                <span className="text-[10px] text-muted-foreground uppercase tracking-widest">Copilot</span>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-80">
              <DropdownMenuLabel className="flex items-center justify-between">
                Notifications <Badge variant="secondary" className="text-[10px]">4 new</Badge>
              </DropdownMenuLabel>
              <DropdownMenuSeparator />
              {[
                { t: "Low stock alert", d: "Sony WH-1000XM5 below safety stock (Mumbai WH)", c: "destructive" },
                { t: "Shipment delayed", d: "SHP-00184 delayed 3 days — weather event", c: "warning" },
                { t: "High fraud risk return", d: "R-00021 flagged by AI — fraud score 0.87", c: "destructive" },
                { t: "Procurement saving", d: "Switch to Quantum Components saves ₹42k/mo", c: "success" },
              ].map((n, i) => (
                <DropdownMenuItem key={i} className="flex flex-col items-start gap-1 py-2.5">
                  <div className="flex items-center gap-2 w-full">
                    <span className={`w-1.5 h-1.5 rounded-full ${
                      n.c === "destructive" ? "bg-destructive" : n.c === "warning" ? "bg-warning" : "bg-success"
                    }`} />
