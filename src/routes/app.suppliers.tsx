...
import { suppliers, CITIES } from "@/lib/mock/data";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { Plus, MapPin, Star, Sparkles, TrendingUp } from "lucide-react";
import {
          ...
          >
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                <div className="font-semibold tracking-tight truncate">{s.supplier_name}</div>
                <div className="text-xs text-muted-foreground inline-flex items-center gap-1 mt-0.5">
                  <MapPin className="w-3 h-3" />{s.city}, {s.state}
                </div>
              </div>
              <Badge variant="secondary" className="text-[10px]">{s.supplier_specialization}</Badge>
            </div>
            <div className="grid grid-cols-3 gap-2 mt-4">
              <Cell k="Reliability" v={`${(s.reliability_score * 100).toFixed(0)}%`} good={s.reliability_score > 0.85} />
              <Cell k="Lead" v={`${s.avg_lead_time_days}d`} good={s.avg_lead_time_days < 8} />
              <Cell k="OTD" v={`${(s.on_time_delivery_rate * 100).toFixed(0)}%`} good={s.on_time_delivery_rate > 0.9} />
            </div>
            <div className="flex items-center justify-between mt-4 pt-3 border-t border-border">
              <div className="flex items-center gap-1 text-xs">
                <Star className="w-3.5 h-3.5 fill-warning text-warning" />
                <span className="font-medium">{(s.reliability_score * 5).toFixed(1)}</span>
                <span className="text-muted-foreground">· {s.payment_terms}</span>
              </div>
