...
function Procurement() {
  ...
  const ranked = suppliers.slice(0, 4).map((s, i) => ({
    ...s,
    landed_cost: Math.round(product.manufacturing_cost * (0.9 + i * 0.06)),
    saving: Math.round(product.manufacturing_cost * (0.1 - i * 0.025) * 100),
    ai_score: +(0.96 - i * 0.08).toFixed(2),
  }));
  ...
  return (
    <>
      ...
      <SectionCard
        title={`AI recommended vendors — ${product.product_name}`}
            ...
            return (
              ...
              >
                ...
                <div className="mt-4 grid grid-cols-2 gap-2 text-xs">
                  <Cell k="Reliability" v={`${(s.reliability_score * 100).toFixed(0)}%`} />
                  <Cell k="Lead" v={`${s.avg_lead_time_days}d`} />
                  <Cell k="Defect" v={`${(s.defect_rate * 100).toFixed(2)}%`} />
                  <Cell k="OTD" v={`${(s.on_time_delivery_rate * 100).toFixed(0)}%`} />
                </div>
          ...
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <RadarChart data={radarData}>
                ...
                <Radar name={ranked[0].supplier_name} dataKey="a" stroke="var(--color-chart-1)" fill="var(--color-chart-1)" fillOpacity={0.35} />
                <Radar name={ranked[1].supplier_name} dataKey="b" stroke="var(--color-chart-3)" fill="var(--color-chart-3)" fillOpacity={0.25} />
                <Radar name={ranked[2].supplier_name} dataKey="c" stroke="var(--color-chart-5)" fill="var(--color-chart-5)" fillOpacity={0.2} />
